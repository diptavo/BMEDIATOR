...
def summarize_group(cell: str, rows: list[dict]) -> dict:
    ...
    return {
        ...
        "m1_sensitivity": rate([row["pair_score"] >= 0.5 for row in m1]) if identifiable else None,
        "m5_specificity": rate([row["pair_score"] < 0.5 for row in m5]) if identifiable else None,
        "all_state_accuracy": rate([
            row.get("pred_scenario") == row["true_scenario"] for row in scored
        ...
        "boundary_m1_pair_call_rate": rate([row["pair_score"] >= 0.5 for row in m5]) if not identifiable else None,
    }
