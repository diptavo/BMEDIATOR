...
config="${2:-$repo/sim/configs/m1_m5_identification_main.json}"
out_root="${3:-/data/Dutta_lab/BMEDIATOR/sim_benchmark/m1_m5_identification}"
array_throttle="${4:-20}"
block_size="${5:-5}"
...
array_job=$(
  sbatch --parsable \
    ...
    --mem=8g \
    --time=03:00:00 \
    -o "$log_dir/bmed_m1m5.%A_%a.out" \
    -e "$log_dir/bmed_m1m5.%A_%a.err" \
    --export=ALL,MANIFEST="$manifest",CONFIG="$config",BMEDIATOR_DIR="$repo",OUT_ROOT="$out_root",BLOCK_SIZE="$block_size" \
    "$repo/scripts/run_m1_m5_array.sh"
)
...
summary_job=$(
  sbatch --parsable \
    ...
    --mem=4g \
    --time=00:30:00 \
    -o "$log_dir/bmed_m1m5_summary.%j.out" \
    -e "$log_dir/bmed_m1m5_summary.%j.err" \
