...
`bmediator-joint` is the supported implementation of the `JG-0.2.4` joint
...
- Replace the order-13 tensor Gauss-Hermite evidence integrator with a more
  reliable and scalable method, then pass a new frozen-seed family calibration
  without relaxing the `0.01` posterior-error criterion. This is the immediate
  blocking defect.
